...
type ChangeEmailInput struct{ UserID uuid.UUID; Email string; Version int }
type ChangePhoneInput struct{ UserID uuid.UUID; Phone string; Version int }
type DeleteUserInput struct{ UserID uuid.UUID; Version int }
type SearchUsersInput struct{ Query string; Limit, Offset int }
...
func (s *Service) GetPublicUserByID(ctx context.Context, id uuid.UUID) (*PublicUserOutput, error) {
	u, err := s.repo.GetByID(ctx, id); if err != nil { return nil, err }; return toPublicUserOutput(u), nil
}
func (s *Service) GetPublicUserByUsername(ctx context.Context, raw string) (*PublicUserOutput, error) {
	uName, err := vo.NewUsername(raw); if err != nil { return nil, fmt.Errorf("%w: %v", ErrInvalidInput, err) }
	u, err := s.repo.GetByUsername(ctx, uName); if err != nil { return nil, err }
	return toPublicUserOutput(u), nil
}
func (s *Service) UpdateProfile(ctx context.Context, in UpdateProfileInput) (*UserOutput, error) {
	u, err := s.repo.GetByID(ctx, in.UserID); if err != nil { return nil, err }
	if u.Version() != in.Version { return nil, entity.ErrVersionConflict }
	if err := u.UpdateProfile(in.DisplayName, in.Bio, in.AvatarURL); err != nil { return nil, fmt.Errorf("%w: %v", ErrInvalidInput, err) }
...
func (s *Service) UpdateSettings(ctx context.Context, userID uuid.UUID, in UpdateSettingsInput) (*UserOutput, error) {
	u, err := s.repo.GetByID(ctx, userID); if err != nil { return nil, err }
	if u.Version() != in.V { return nil, entity.ErrVersionConflict }
	...
	if err := u.UpdateSettings(settings); err != nil { return nil, fmt.Errorf("%w: %v", ErrInvalidInput, err) }
	if err := s.repo.Update(ctx, u); err != nil { return nil, err }
	return toUserOutput(u), nil
}
func (s *Service) ChangeEmail(ctx context.Context, in ChangeEmailInput) (*UserOutput, error) {
	e, err := vo.NewEmail(in.Email); if err != nil { return nil, fmt.Errorf("%w: %v", ErrInvalidInput, err) }
	u, err := s.repo.GetByID(ctx, in.UserID); if err != nil { return nil, err }
	if u.Version() != in.Version { return nil, entity.ErrVersionConflict }
	existing, err := s.repo.GetByEmail(ctx, e); if err != nil && !errors.Is(err, entity.ErrUserNotFound) { return nil, err }
	if existing != nil && existing.ID() != u.ID() { return nil, entity.ErrEmailAlreadyTaken }
	u.ChangeEmail(e); if err := s.repo.Update(ctx, u); err != nil { return nil, err }; return toUserOutput(u), nil
}
func (s *Service) ChangePhone(ctx context.Context, in ChangePhoneInput) (*UserOutput, error) {
	p, err := vo.NewPhone(in.Phone); if err != nil { return nil, fmt.Errorf("%w: %v", ErrInvalidInput, err) }
	u, err := s.repo.GetByID(ctx, in.UserID); if err != nil { return nil, err }
	if u.Version() != in.Version { return nil, entity.ErrVersionConflict }
	existing, err := s.repo.GetByPhone(ctx, p); if err != nil && !errors.Is(err, entity.ErrUserNotFound) { return nil, err }
	if existing != nil && existing.ID() != u.ID() { return nil, entity.ErrPhoneAlreadyTaken }
	u.ChangePhone(p); if err := s.repo.Update(ctx, u); err != nil { return nil, err }; return toUserOutput(u), nil
}
func (s *Service) DeleteUser(ctx context.Context, in DeleteUserInput) error {
	u, err := s.repo.GetByID(ctx, in.UserID); if err != nil { return err }
	if u.Version() != in.Version { return entity.ErrVersionConflict }
	if err := u.SoftDelete(); err != nil { return err }
	return s.repo.Update(ctx, u)
}
func (s *Service) UpdateLastSeen(ctx context.Context, userID uuid.UUID) error { return s.repo.UpdateLastSeen(ctx, userID) }
func (s *Service) SearchUsers(ctx context.Context, in SearchUsersInput) ([]*PublicUserOutput, error) {
	q := strings.TrimSpace(in.Query); if q == "" { return nil, fmt.Errorf("%w: empty query", ErrInvalidInput) }
	limit := in.Limit; if limit <= 0 { limit = 20 }; if limit > 100 { limit = 100 }
	offset := in.Offset; if offset < 0 { offset = 0 }
	users, err := s.repo.Search(ctx, q, limit, offset); if err != nil { return nil, err }
...
func (s *Service) GetUsersByIDs(ctx context.Context, ids []uuid.UUID) ([]*PublicUserOutput, error) {
	if len(ids) == 0 { return []*PublicUserOutput{}, nil }
	if len(ids) > 500 { return nil, fmt.Errorf("%w: batch size exceeds 500", ErrInvalidInput) }
	users, err := s.repo.GetByIDs(ctx, ids); if err != nil { return nil, err }
	return toPublicUserOutputs(users), nil
}
...
func toPublicUserOutputs(users []*entity.User) []*PublicUserOutput {
	...
	for _, u := range users { out = append(out, toPublicUserOutput(u)) }
	return out
}
